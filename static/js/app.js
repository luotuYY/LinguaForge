...
// ── 对比表分词 ──
var _tagCompareRunning = false;
async function tagCompareItems() {
  if (_tagCompareRunning) { showToast('分词进行中，请等待完成'); return; }
  // 取当前可见的对比表条目（有 new_translation 或 error 的行）
  var rows = state.lines.filter(function (l) { return l.new_translation || l.error; });
  if (rows.length === 0) { showToast('对比表没有可分词的条目'); return; }
  // 筛选未分词的
  var pending = rows.filter(function (l) { return !l.tag_l1; });
  if (pending.length === 0) {
    // 全部已分词 → 清空重来
    rows.forEach(function (l) { l.tag_l1 = ''; l.tag_l2 = ''; l.confidence = 0; });
    pending = rows.slice();
  }

  _tagCompareRunning = true;
  var btn = $('btnTagCompare');
  if (btn) { btn.disabled = true; btn.textContent = '分词中...'; }

  var apiConfig = getApiConfig();
  var llmParams = getLLMParams();
  var schema = dbGet('tllmh_tag_schema');
  var schemaForBadge = null;
  if (schema && typeof schema === 'object' && Object.keys(schema).length > 0) {
    schemaForBadge = schema;
  } else {
    schemaForBadge = {
      '硬术语': { color: '#4fc3f7', icon: '🔧', subs: ['UI文本','菜单/按钮','属性/状态','物品/装备','技能/招式','系统提示','Mod/插件','代码标识符'] },
      '硬生动': { color: '#81c784', icon: '🎭', subs: ['对话/台词','旁白/叙述','情感/语气','俚语/口语','描述/刻画','幽默/讽刺','严肃/正式'] }
    };
  }
  window._compareTagSchema = schemaForBadge;

  var strategyText = '你是一个游戏文本分类专家。请将以下文本归入最合适的类别。';
  var catDesc = '';
  var enabledSchema = {};
  Object.keys(schemaForBadge).forEach(function(k) {
    if (schemaForBadge[k].enabled !== false) enabledSchema[k] = schemaForBadge[k];
  });
  Object.keys(enabledSchema).forEach(function(l1) { catDesc += l1 + ': ' + enabledSchema[l1].subs.join(', ') + '\n'; });
  var fullPrompt = strategyText + '\n\n可用类别(一级 / 二级):\n' + catDesc + '\n请严格输出以下JSON格式:{"l1":"一级类目","l2":"二级类目","confidence":0.0~1.0}\n只输出JSON,不要其他内容。';

  var concurrency = parseInt($('concurrency') && $('concurrency').value) || 5;
  var chunks = [];
  ...
  log('对比表分词: ' + total + ' 条,并发 ' + concurrency + ',分 ' + chunks.length + ' 块');

  try {
    for (var chunkIdx = 0; chunkIdx < chunks.length; chunkIdx++) {
      var chunk = chunks[chunkIdx];
      // 标记分词中
      chunk.forEach(function (l) { l._tagging = true; });
      renderCompare();
      ...
      var batchBody = Object.assign({
        ...
        temperature: llmParams.temperature || 0.1,
        top_p: llmParams.top_p || 0.6,
        max_tokens: llmParams.max_tokens || 512,
      }, apiConfig);
      ...
      try {
        ...
        if (!r.ok) {
          ...
          chunk.forEach(function (l) { l._tagging = false; });
          log('块 ' + (chunkIdx + 1) + ' 失败: HTTP ' + r.status, 'err');
          continue;
        }
            ...
            try {
              ...
              if (pos >= 0 && pos < chunk.length) {
                ...
                if (res.error) {
                  ...
                  if (l1) {
                    var schemaKeys = Object.keys(enabledSchema);
                    ...
                    if (matchL1) { l1 = matchL1; }
                      ...
                      schemaKeys.forEach(function(k) {
                        if (enabledSchema[k].subs.indexOf(l1) >= 0) foundParent = k;
                      });
                      if (foundParent) { if (!l2) l2 = l1; l1 = foundParent; }
                      else {
          ...
          try {
            ...
            if (lastPos >= 0 && lastPos < chunk.length) {
              ...
              if (lastRes.error) { errors++; }
                ...
                if (l1r) {
                  var sk = Object.keys(enabledSchema);
        ...
        chunk.forEach(function (l) { l._tagging = false; });
        log('块 ' + (chunkIdx + 1) + ' 异常: ' + fetchErr.message, 'err');
      }
    }
  } catch (e) {
    log('分词异常: ' + e.message, 'err');
  }

  _tagCompareRunning = false;
  if (btn) { btn.disabled = false; btn.textContent = '分词'; }
  renderCompare();
  log('对比表分词完成: 成功' + (done - errors) + '条' + (errors ? ' · 失败' + errors + '条' : ''), errors ? 'err' : 'ok');
  showToast('分词完成: 成功' + (done - errors) + '条' + (errors ? ', 失败' + errors + '条' : ''));
}
...
(function () {
  ...
  _bind('btnTagCompare', 'click', tagCompareItems);
